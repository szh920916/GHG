...
public partial class Modules_GHG_SystemManage_PaiFangBaoGaoJinDu : System.Web.UI.Page
{
    ...
    protected void bindDatelb()
    {
        ...
        conn.Open();
        string sql = "select count(*) from HeChaBaoGaoXinXi";
        SqlCommand cmd = new SqlCommand(sql, conn);
        this.Label2.Text = cmd.ExecuteScalar().ToString();

        string sql1 = "select count(*) from Company";
        SqlCommand cmd1 = new SqlCommand(sql1, conn);
        this.Label4.Text = (Convert.ToInt32(cmd1.ExecuteScalar().ToString()) - Convert.ToInt32(cmd.ExecuteScalar().ToString())).ToString() ;
    }
